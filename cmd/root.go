...
Complete documentation is available at:
  https://github.com/cristiangonsevi/orbit`,
	...
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		...
		return []string{"init", "list", "run", "update", "validate", "completion", "version"}, cobra.ShellCompDirectiveNoFileComp
	},
}
...
_orbit_completion() {
    ...
    if [[ $cword -eq 1 ]]; then
        COMPREPLY=($(compgen -W "init list run update validate completion version" -- "$cur"))
        return 0
    fi
